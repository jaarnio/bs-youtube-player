...
(function (global, factory) {
  ...
  const registerPlugin =
    videojs__default["default"].registerPlugin || videojs__default["default"].plugin; // Default options for the plugin.
  ...
  const defaults = {
    ...
    errors: {
      ...
      2: {
        ...
        headline:
          "The video connection was lost, please confirm you are " +
      ...
      5: {
        ...
        headline:
          "The video you are trying to watch is encrypted and we do not know how " +
          "to decrypt it",
      },
      unknown: {
      ...
      "-2": {
        ...
        headline:
          "It looks like you're having playback issues. Reloading the video may help.",
      },
  ...
  const initPlugin = function (player, options) {
    ...
    const resetMonitor = function () {
      ...
      if (
        ...
        (document.visibilityState === "visible" &&
          disableValueMatch(disableValues, options.timeout))
      ) {
        return;
    ...
    const onErrorHandler = function () {
      ...
      player.off(document, "visibilitychange", onPlayStartMonitor);
      error = videojs__default["default"].mergeOptions(
        error,
        options.errors[error.code || error.type || 0]
      );
      ...
      const errorCode = `<div class="vjs-errors-code"><b>${this.localize(
        "Error Code"
      )}:</b> ${error.type || error.code}</div>`;
      ...
      if (isTimeoutError) {
        ...
        const reloadButton = display
          .el()
          .querySelector(".vjs-errors-timeout-button-container > button:first-child");
        const dismissButton = display
          .el()
          .querySelector(".vjs-errors-timeout-button-container > button:last-child");
  ...
  ["extend", "getAll"].forEach((k) => {
    errors[k] = function () {
      videojs__default["default"].log.warn(
